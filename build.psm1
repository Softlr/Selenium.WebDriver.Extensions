...
function Publish-Coveralls {
    ...
    $coverallsPath = Resolve-Path .\packages\coveralls.io.*\tools\coveralls.net.exe
    Exec {
          & $coverallsPath --opencover $CoverageXml -f
    }
}
...
function Test-Assembly {
    ...
    $xunitPath = Resolve-Path .\packages\xunit.runner.console.*\tools\xunit.console.exe
    Exec {
        & $xunitPath $testAssemblies -noshadow -parallel all
    }
}
